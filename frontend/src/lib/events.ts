/**
 * Поток обновлений списка устройств в десктопной сборке.
 *
 * Замена WebSocket `/ws`: Rust-лаунчер рассылает событие `devices_update` с тем
 * же payload, что слал сокет (`{type, devices, timestamp}`), поэтому обработчик
 * в `App.tsx` не менялся — поменялся только способ подписки.
 *
 * В браузере веб-панели этот модуль не используется: там остаётся настоящий
 * WebSocket, и код для него в Python сохранён.
 */
