...
/// перезапросит список (при живом WebSocket — с его пуша, иначе — поллингом
/// раз в 5 секунд).
...
pub async fn list_connected(adb: &Path) -> Result<Vec<ConnectedDevice>, ApiError> {
    ...
    command
        .arg("devices")
        .stdin(Stdio::null())
        // Команда вызывается регулярно; без этого каждый таймаут оставлял бы
        // после себя осиротевший процесс adb — tokio сам их не убивает.
        .kill_on_drop(true);
