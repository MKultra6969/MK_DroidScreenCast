...
//!
//! Прежняя версия на Python той же беды не знала: PyInstaller собирал её
//! **консольным** приложением, а `CREATE_NO_WINDOW` даёт консольному
//! приложению безоконную консоль, которую scrcpy наследовал, — сигнал доходил.
//! Это приложение собрано с `windows_subsystem = "windows"`, консоли у него
//! нет вообще, и дословный перенос того кода сломал бы остановку записи,
//! причём молча. Отсюда и вся возня ниже: не «упростить» обратно.
//!
