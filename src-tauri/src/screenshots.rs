...
//! время и серийник устройства. Его же читает и пишет Python, поэтому формат
//! файла менять нельзя.
//!
//! Одно поле в ответе добавлено сверх REST — `path`, абсолютный путь к файлу
//! на диске. По HTTP картинка отдаётся эндпоинтом `GET /api/screenshots/{id}`,
//! но в десктопе URL взять неоткуда: там фронтенд превращает этот путь в
//! `asset://` через `convertFileSrc`. Веб-панель поля не видит и продолжает
//! ходить за картинкой по HTTP.
