...
//!
//! Здесь только поиск уже лежащего на диске бинаря. Скачивание, проверка
//! контрольных сумм и биты исполнения — в `install.rs`. Порядок поиска
//! унаследован от `mkdsc/tools.py` и намеренно не менялся: по нему
//! пользовательские установки находят ровно тот adb, что и раньше.
